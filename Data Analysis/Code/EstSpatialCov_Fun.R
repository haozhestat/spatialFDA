EstSpatialCov_Fun <- function(Est3DCov_output, EstFPC_output){
  ...
  SpatialCov_Fun <- function(s, k){
    sbs <- bs(x = s, knots = s_knots, intercept=TRUE, degree = deg_s, Boundary.knots = c(0, Delta))
    t1bs <- c(t(EstFPC_output$EstPhi_beta[,k])%*%jmat)
    t2bs <- c(t(EstFPC_output$EstPhi_beta[,k])%*%jmat)
  ...
  return(list(SpatialCov_Fun = SpatialCov_Fun))
}
