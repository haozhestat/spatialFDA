...
data <- data.frame(NA, length(fpcaObjFlies$phi[,1]),nrow(fpcaObjFlies$xiEst))
for(j in 1:nrow(location)){
  data[,j] <- fpcaObjFlies$xiEst[j,1]) * fpcaObjFlies$phi[,1] +
              fpcaObjFlies$xiEst[j,2]) * fpcaObjFlies$phi[,2] +
              fpcaObjFlies$xiEst[j,3]) * fpcaObjFlies$phi[,3]
}
