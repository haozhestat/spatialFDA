...
#setwd("~/Dropbox/projects/SPCA/Simulation A")
...
foreach(i=1:num_rep, .combine=list,.packages=c("fdapace","geoR","fdagstat")) %dopar% {
  ...
  #train <- sample(1:nrow(sim_data_location), 100)
  #test <- setdiff(1:nrow(sim_data_location), train)
  ...
  sample_index <- sample(1:nrow(sim_data_location), 20)
  
  for(test in sample_index){
    ...
    true_curve <- mean_fun(Grid) +
      phi1_fun(Grid)*sim_data_fpcscore$score1[test]+
      phi2_fun(Grid)*sim_data_fpcscore$score2[test]+
      phi3_fun(Grid)*sim_data_fpcscore$score3[test]
    
    pred_curve <- fpcaObjFlies$mu +
      forecast.pc1$Forecast[1,1]*fpcaObjFlies$phi[,1]+
      forecast.pc2$Forecast[1,1]*fpcaObjFlies$phi[,2]+
      forecast.pc3$Forecast[1,1]*fpcaObjFlies$phi[,3]+
      forecast.pc4$Forecast[1,1]*fpcaObjFlies$phi[,4]+
      forecast.pc5$Forecast[1,1]*fpcaObjFlies$phi[,5]
    
    pred_error <- c(pred_error, mean((true_curve - pred_curve)^2))
    
    write.csv(data.frame(pred_error = pred_error),
              file=paste0("CoKriging_output/CoKriging_pred_error_", i,".csv"),
              row.names = FALSE)
  }
}
stopCluster(cl)
