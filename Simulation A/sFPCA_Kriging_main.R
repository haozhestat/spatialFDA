...
foreach(i=1:200, .combine=list,.packages=c("splines")) %dopar% {
tryCatch({
  ...
  for(j in 1:test){
    ...
    write.csv(pred_xi,
              file=paste0("spatialpace_output/spatialpace_output_",i,".csv"),
              row.names = FALSE)
    write.csv(pred_error,
              file=paste0("spatialpace_output/spatialpace_pred_error_",i,".csv"),
              row.names = FALSE)
    write.csv(baseline_error,
              file=paste0("spatialpace_output/spatialpace_baseline_error_",i,".csv"),
              row.names = FALSE)
  }
