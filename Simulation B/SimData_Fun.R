SimData_Fun <- function(lambda_spatial = 10,
  ...
  z1_data <- RFsimulate(model = RMtrend(mean=0) + RMmatern(nu = nu1, var=omega1),
                        x=x_coord, y=y_coord, grid=FALSE)$variable1
  z2_data <- RFsimulate(model = RMtrend(mean=0) + RMmatern(nu = nu2, var=omega2),
                        x=x_coord, y=y_coord, grid=FALSE)$variable1
  z3_data <- RFsimulate(model = RMtrend(mean=0) + RMmatern(nu = nu3, var=omega3),
                        x=x_coord, y=y_coord, grid=FALSE)$variable1
